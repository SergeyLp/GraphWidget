...
void GraphWidget::setCurveData(int id, const QVector<QPointF> &data)
{
    curveMap[id] = data;
    refreshPixmap();
}

void GraphWidget::clearCurve(int id)
{
    curveMap.remove(id);
    refreshPixmap();
}
...
void GraphWidget::refreshPixmap()
{
    pixmap = QPixmap(size());
...
void GraphWidget::drawGrid(QPainter *painter){
    const QRect rect(Margin, Margin, width() - 2 * Margin, height() - 2 * Margin);
    if (!rect.isValid())
        return;

    const PlotSettings settings = zoomStack[curZoom];
    const QPen quiteDark = palette().dark().color().light();
    const QPen light = palette().light().color();
    ...
    for (int i = 0; i <= settings.numXTicks; ++i) {
        ...
        painter->drawText(x - 50, rect.top() - 15, 100, 20,
            Qt::AlignHCenter | Qt::AlignTop,
            QString::number(label));
    }
    for (int j = 0; j <= settings.numYTicks; ++j) {
        ...
        painter->drawText(rect.left() - Margin, y - 10, Margin - 5, 20,
                          ...
                          QString::number(label));
        painter->drawText(rect.right()+2, y - 10, Margin - 5, 20,
